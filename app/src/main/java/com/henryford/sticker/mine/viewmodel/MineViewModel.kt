...
import com.henryford.sticker.util.RxJavaUtil
...
class MineViewModel : ViewModel() {
    ...
    var stickerList = MutableLiveData<ArrayList<MineStickerBean>>()
    fun getMineData(){
    ...
    fun getStickerList(){
        Observable.create<ArrayList<MineStickerBean>> {
            var list : ArrayList<MineStickerBean> =
                AppDataBase.getInstance(StickerApplication.context).mineStickerDao().queryAll() as ArrayList<MineStickerBean>
            it.onNext(list)
            it.onComplete()
        }.compose(RxJavaUtil.observableIoToMain())
            .subscribe {
                stickerList.value = it
            }
    }
}